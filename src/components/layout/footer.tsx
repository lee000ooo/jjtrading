import Link from "next/link";
import { Cpu, Mail, Phone, MapPin, Clock, ArrowUpRight } from "lucide-react";
import type { Locale } from "@/lib/utils";
import { asset } from "@/lib/asset";

interface FooterProps {
  commonDict: Record<string, string>;
  locale: Locale;
}

export function Footer({ commonDict, locale }: FooterProps) {
  const quickLinks = [
    { href: `/${locale}`, label: commonDict["nav.home"] },
    { href: `/${locale}/about`, label: commonDict["nav.about"] },
    { href: `/${locale}/products`, label: commonDict["nav.products"] },
    { href: `/${locale}/partner/shuofeng`, label: commonDict["nav.partner.shuofeng"] },
    { href: `/${locale}/partner/youchang`, label: commonDict["nav.partner.youchang"] },
    { href: `/${locale}/contact`, label: commonDict["nav.contact"] },
  ];

  return (
    <footer className="border-t bg-surface-950 text-surface-50">
      <div className="container mx-auto px-4 max-w-7xl">
        {/* Main Footer */}
        <div className="py-12 md:py-16">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
            {/* Company Info */}
            <div className="lg:col-span-1">
              <Link
                href={`/${locale}`}
                className="flex items-center gap-3 mb-4"
              >
                <img
                  src={asset("/images/logo.png")}
                  alt="璟江贸易"
                  className="h-10 w-auto brightness-0 invert"
                />
                <span className="text-sm font-bold leading-tight text-white">
                  璟江<span className="text-primary">贸易</span>
                </span>
              </Link>
              <p className="text-surface-400 text-sm leading-relaxed mb-6">
                {commonDict["footer.tagline"]}
              </p>
              <div className="flex gap-3">
                <a
                  href="mailto:lichengyin@jjtrading.com.cn"
                  className="flex items-center justify-center w-10 h-10 rounded-lg bg-surface-800 text-surface-400 hover:bg-primary hover:text-white transition-colors"
                  aria-label="Email"
                >
                  <Mail className="h-4 w-4" />
                </a>
                <a
                  href="https://t.me/lcy_jjtrading"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center w-10 h-10 rounded-lg bg-surface-800 text-surface-400 hover:bg-primary hover:text-white transition-colors"
                  aria-label="Telegram"
                >
                  <svg
                    viewBox="0 0 24 24"
                    fill="currentColor"
                    className="h-4 w-4"
                  >
                    <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
                  </svg>
                </a>
                <a
                  href="https://wa.me/8613580353907"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center w-10 h-10 rounded-lg bg-surface-800 text-surface-400 hover:bg-primary hover:text-white transition-colors"
                  aria-label="WhatsApp"
                >
                  <svg
                    viewBox="0 0 24 24"
                    fill="currentColor"
                    className="h-4 w-4"
                  >
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                  </svg>
                </a>
              </div>
            </div>

            {/* Quick Links */}
            <div>
              <h3 className="text-sm font-semibold uppercase tracking-wider text-surface-300 mb-4">
                {commonDict["footer.quickLinks"]}
              </h3>
              <ul className="space-y-3">
                {quickLinks.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="flex items-center gap-1 text-sm text-surface-400 hover:text-white transition-colors group"
                    >
                      <ArrowUpRight className="h-3 w-3 opacity-0 -translate-y-1 group-hover:opacity-100 group-hover:translate-y-0 transition-all" />
                      <span>{link.label}</span>
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* Product Categories */}
            <div>
              <h3 className="text-sm font-semibold uppercase tracking-wider text-surface-300 mb-4">
                Продукция
              </h3>
              <ul className="space-y-3">
                <li>
                  <Link
                    href={`/${locale}/products`}
                    className="text-sm text-surface-400 hover:text-white transition-colors"
                  >
                    Интегральные микросхемы
                  </Link>
                </li>
                <li>
                  <Link
                    href={`/${locale}/products`}
                    className="text-sm text-surface-400 hover:text-white transition-colors"
                  >
                    Микроконтроллеры
                  </Link>
                </li>
                <li>
                  <Link
                    href={`/${locale}/products`}
                    className="text-sm text-surface-400 hover:text-white transition-colors"
                  >
                    Датчики и сенсоры
                  </Link>
                </li>
                <li>
                  <Link
                    href={`/${locale}/products`}
                    className="text-sm text-surface-400 hover:text-white transition-colors"
                  >
                    Силовая электроника
                  </Link>
                </li>
                <li>
                  <Link
                    href={`/${locale}/products`}
                    className="text-sm text-surface-400 hover:text-white transition-colors"
                  >
                    Пассивные компоненты
                  </Link>
                </li>
              </ul>
            </div>

            {/* Contact Info */}
            <div>
              <h3 className="text-sm font-semibold uppercase tracking-wider text-surface-300 mb-4">
                {commonDict["footer.contactInfo"]}
              </h3>
              <ul className="space-y-4">
                <li className="flex items-start gap-3">
                  <MapPin className="h-4 w-4 text-primary mt-0.5 shrink-0" />
                  <span className="text-sm text-surface-400">
                    {commonDict["footer.address"]}
                  </span>
                </li>
                <li>
                  <a
                    href="tel:+8613580353907"
                    className="flex items-center gap-3 text-sm text-surface-400 hover:text-white transition-colors"
                  >
                    <Phone className="h-4 w-4 text-primary shrink-0" />
                    <span>{commonDict["footer.phone"]}</span>
                  </a>
                </li>
                <li>
                  <a
                    href="mailto:lichengyin@jjtrading.com.cn"
                    className="flex items-center gap-3 text-sm text-surface-400 hover:text-white transition-colors"
                  >
                    <Mail className="h-4 w-4 text-primary shrink-0" />
                    <span>{commonDict["footer.email"]}</span>
                  </a>
                </li>
                <li className="flex items-start gap-3">
                  <Clock className="h-4 w-4 text-primary mt-0.5 shrink-0" />
                  <span className="text-sm text-surface-400">
                    {commonDict["footer.workingHours"]}
                  </span>
                </li>
              </ul>
            </div>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="py-6 border-t border-surface-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-surface-500">
            {commonDict["footer.rights"]}
          </p>
          <div className="flex gap-6 text-xs text-surface-500">
            <span>Чипы, полупроводники, электронные компоненты</span>
          </div>
        </div>
      </div>
    </footer>
  );
}
